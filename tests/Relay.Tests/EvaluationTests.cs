...
/// scores a planner and the mind against recorded, unseen, and failure cases, refusing a set in which
...
public class EvaluationTests : IDisposable
{
    ...
    /// <summary>The model-primary set: delegation cases and the mind's own moves, written for a model; the grammar is never asked to pass it.</summary>
    ...
    public async Task RecordedCasesAloneAreRefusedAndTheGuardNamesEveryGap()
    {
        ...
        [
            ...
            new EvaluationCase { Id = "f2", Source = CaseSources.Failure, Origin = "observed", Expect = new Expectation { Actions = [] }, Why = "x" },                                  // no instruction
    ...
    /// <summary>The whole harness: recorded cases from real task diagnostics plus the authored sets, scored against the rules planner.</summary>
    [Fact]
    public async Task TheRulesPlannerPassesTheWholeSet()
    {
        ...
        var runner = new EvaluationRunner(new RuleBasedOrchestrator(), c => s.H.PlannerContext(c), () => s.H.Clock.UtcNow);
    ...
    public async Task AFailingCaseIsReportedWithItsReasonsAndWhatWasObserved()
    {
        ...
        var report = await new EvaluationRunner(shrug, c => s.H.PlannerContext(c)).RunAsync(authored, CancellationToken.None);
        Assert.False(report.Passed);
        ...
        // Failure cases that only forbid actions still pass for a planner that proposes nothing.
        ...
        var throwing = await new EvaluationRunner(new ThrowingOrchestrator(), c => s.H.PlannerContext(c)).RunAsync(authored, CancellationToken.None);
        ...
        var hanging = new EvaluationRunner(new HangingOrchestrator(), c => s.H.PlannerContext(c)) { CaseTimeout = TimeSpan.FromMilliseconds(50) };
        var subset = new EvaluationSet(authored.Of(CaseSources.Unseen).Where(c => !c.IsMindCase).Take(2).Concat(authored.Of(CaseSources.Failure).Where(c => !c.IsMindCase).Take(1)));
    ...
    /// <summary>
    /// An observed plan case carries the words the mind kept ("heard"); the runner hands the planner an excerpt id and the
    ...
    /// <summary>
    /// The model-primary set is complete on its own and is written only for a model (every case is prefixed so
    /// the report names the set).
    /// </summary>
    [Fact]
    public void TheModelPrimarySetIsComplete()
    {
        var set = ModelPrimary();
        Assert.True(set.Cases.Count >= 8, $"expected the model-primary sets, found {set.Cases.Count} case(s) under {Path.Combine(CasesDirectory, "model")}");
        Assert.Empty(set.Validate());
        ...
        var plan = set.Cases.Where(c => !c.IsMindCase).ToList();
        Assert.True(plan.Count >= 8, $"plan {plan.Count}");
    ...
    /// keeps the report). The model plans every plan case with no grammar in front of it. Accuracy is reported, not
    /// asserted; what is asserted is that the run happened with the model alone. Never runs in CI.
    /// </summary>
    [Fact]
    public async Task TheLiveModelPlannerIsScoredAsThePrimaryTool()
    {
        ...
        var runner = new EvaluationRunner(new ModelOrchestrator(client), c => s.H.PlannerContext(c)) { CaseTimeout = TimeSpan.FromSeconds(180) };
        var report = await runner.RunAsync(set, CancellationToken.None);
    ...
    public async Task MindCasesScoreTheLoopsMovesAndStopAtTheFirstThingThatNeedsTheUser()
    {
        ...
        Assert.Empty(Authored().With(set.Cases).Validate());                                           // one valid set together with the planner cases
    ...
    /// <summary>
    /// The live chain with the model in both seats: words are overheard, the model's own listening loop raises the tasks,
    /// the model planner reads the record and proposes, policy holds the proposals for approval, and a direct research ask
    /// is delegated to a named external agent (scripted here: it is the counterparty, not what is under test). Runs only
    /// with RELAY_LIVE_MODEL_KEY. The invariants are asserted; the transcript, written to RELAY_LIVE_REPORT_DIR, shows
    /// what the model made of each sentence.
    /// </summary>
    [Fact]
    public void TheLiveMindRaisesObservedTasksAndTheModelPlannerActsOnThem()
    {
        var live = LiveModel.FromEnvironment();
        if (live is null) return;
        using var client = live.Client();
        ...
        using var s = Scenario.New(_tmp, cfg => { live.Configure(cfg); WithResearchProfile(cfg); }, orchestrator: planner, mind: new ModelMind(client), externalClients: _ => external, inlinePost: false)
        ...
        try
        {
            ...
            Assert.All(observed.Where(t => t.Kind != TaskKind.Remember || t.Producer != Producers.Mind), t => Assert.StartsWith("model:", t.Producer));
