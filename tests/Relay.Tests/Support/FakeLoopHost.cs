...
public sealed class FakeLoopHost : ILoopHost
{
    ...
    public Func<RunWorkflowMove, MoveOutcome> OnRunWorkflow { get; set; }
    ...
    public FakeLoopHost()
    {
        ...
        OnRunWorkflow = m => MoveOutcome.Of(new WorkflowObserved(_at, m.Name, WorkflowObserved.Finished, "ok"));
    ...
    public Task<MoveOutcome> RunWorkflowAsync(TaskLoop loop, RunWorkflowMove move, CancellationToken ct) { Calls.Add("run_workflow:" + move.Name); return Task.FromResult(OnRunWorkflow(move)); }
