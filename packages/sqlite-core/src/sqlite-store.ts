...
export class SqliteEngineStore implements EngineStore {
  ...
  static fromHandle(db: SqlHandle, artifacts?: ArtifactStorePort): SqliteEngineStore {
    return new SqliteEngineStore(db, artifacts);
  }
  ...
  static async openHandle(db: SqlHandle, artifacts?: ArtifactStorePort): Promise<SqliteEngineStore> {
    const store = SqliteEngineStore.fromHandle(db, artifacts);
    if (artifacts) {
...
function applyMigrations(db: SqlHandle): void {
  const now = new Date().toISOString();
  db.exec("BEGIN");
  try {
    const first = MIGRATION_SQL[1];
