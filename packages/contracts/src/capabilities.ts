...
export const V1_CAPABILITY_MATRIX: readonly CapabilityRow[] = Object.freeze([
  ...
  },
  {
    ...
    ios: "not-shipped",
    android: "not-shipped",
    notes: "External loopback llama.cpp-compatible server; fails closed when absent. An invalid typed draft is repaired once. Not verified on a headed Windows release profile.",
  },
  {
  ...
  },
  {
    ...
    ios: "degraded",
    android: "degraded",
    notes: "Transport, jev-latest, a scoped disclosure grant, a two-round cap, and one durable resume exist in code. Judgment events keep the grant id, HTTP status, retry delay, and provider request id when one is returned. Mobile uses that typed port. The unused pending transport is removed. Live canary has not run.",
  },
  {
    id: "memory.local",
