...
*Closing: four of the six commits below have landed. `mind` is the default and the mind reads the stream itself; `IJudge` and both its implementations are gone; the mind now produces the consistency verdict and the citations that only a plan carried, so the README's flagship workflow runs end to end on it. The grammar is still in the tree, and still plans a direct ask.*
