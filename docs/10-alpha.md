...
*Closing: five of the six commits below have landed. `mind` is the default and the mind reads the stream itself; `IJudge` and both its implementations are gone; the mind produces the consistency verdict and the citations that only a plan carried, so the README's flagship workflow runs end to end on it; and the grammar, the model orchestrator, `IOrchestrator` and `CompositeOrchestrator` are deleted. `orchestrator.mode` is now `off` or `mind`, and with the mode on and no model a task fails visibly rather than falling back. Only the state collapse is left.*
