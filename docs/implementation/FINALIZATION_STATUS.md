...
| F4 UI lifecycle quality | CODE LANDED | — | Boot surfaces at `5f83b2f`. App-held composer draft at `4634fff` | Not GREEN: visual, keyboard, screen-reader, device a11y |
...
| G4 UI and developer console | OPEN | Boot surfaces at `5f83b2f`. Console filters, copy, and export at `d0cca3b`. Composer draft retained on a refused send at `27aeabf`. The app holds that draft at `4634fff`. Still open: visual matrix, keyboard-only Windows, screen reader, screenshot diffs |
