...
Windows V1 wrap-up is on branch `wrap/windows-v1-final` (baseline `88d5437`). Architecture authority remains:
...
| Jev status chip | Evidence-based: `missing key` → `hosted off` → `configured` → `ready` / `degraded` (periodic refresh does not invent ready) |
| Listen | Tauri `audio_*` commands + `relay_audio.live` NDJSON source; mic finals require Listening ON |
| Replay | Developer fixture replay uses `ingestReplayFinalSegment` — does **not** set Listening or start audio |
...
- Manual Windows dogfood checklist should still be exercised on a machine with model + optional ASR before calling the gate “shipped”
